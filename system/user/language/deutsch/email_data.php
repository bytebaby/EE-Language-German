...
if ( ! function_exists('admin_notify_reg')) {
	...
	{
return <<<EOF
Registrierungsseite für neue Mitglieder: {site_name}

Bildschirmname: {name}
Benutzername: {username}
Email: {email}

Ihr Control Panel URL: {control_panel_url}
EOF;
	}
}
...
if ( ! function_exists('mbr_activation_instructions'))
{
	...
	{
return <<<EOF
Vielen Dank für Ihre Registrierung als neues Mitglied.

Um Ihr neues Konto zu aktivieren, klicken Sie bitte auf folgenden Link: 
...
Um Ihr Passwort zurückzusetzen, gehen Sie bitte auf die folgende Seite:

{reset_url}

Dann melden Sie sich mit Ihrem Benutzernamen an: {username}

Wenn Sie Ihr Passwort nicht zurücksetzen möchten, ignorieren Sie diese Meldung. Sie läuft in 24 Stunden ab.
...
Wenn Sie diese Änderung nicht selbst vorgenommen haben, wenden Sie sich sofort an einen Administrator.
...
Ihr Mitgliedskonto wurde aktiviert und ist einsatzbereit.
...
Es tut uns leid, aber unsere Administrator hat beschlossen, Ihre Mitgliedschaft nicht zu bestätigen.
...
Sie können den Kommentar unter der folgenden URL sehen:
{comment_url}

{comment}

Klicken Sie hier, um keine Benachrichtigungen für diesen Kommentar mehr zu erhalten:
...
if ( ! function_exists('comments_opened_notification_title'))
{
	...
	{
return <<<EOF
Neue Kommentare wurden hinzugefügt
EOF;
	}
}
...
if ( ! function_exists('comments_opened_notification'))
{
	...
	{
return <<<EOF
Dem Eintrag, den Sie abonniert haben, wurden Antworten hinzugefügt:
{channel_name}

Der Titel des Eintrags lautet:
{entry_title}

Sie können die Kommentare unter der folgenden URL sehen:
{comment_url}
...
Klicken Sie hier, um keine Benachrichtigungen für diesen Eintrag mehr zu erhalten:
{notification_removal_url}
EOF;
	}
}
...
if ( ! function_exists('admin_notify_forum_post_title'))
{
	...
	{
return <<<EOF
Jemand hat gerade gepostet {forum_name}
EOF;
	}
}
...
if ( ! function_exists('admin_notify_forum_post'))
{
	...
	{
return <<<EOF
{name_of_poster} hat gerade einen neuen Beitrag in {forum_name} eingereicht
...
if ( ! function_exists('forum_post_notification_title'))
{
	...
	{
return <<<EOF
Jemand hat gerade gepostet in {forum_name}
EOF;
	}
}
...
if ( ! function_exists('forum_post_notification'))
{
	...
	{
return <<<EOF
Jemand hat gerade in einem Thread gepostet, den Sie abonniert haben:
{forum_name}
...
if ( ! function_exists('private_message_notification_title'))
{
	...
	{
return <<<EOF
Jemand hat Ihnen eine private Nachricht gesendet
EOF;
	}
}
...
{sender_name} hat Ihnen gerade eine private Nachricht mit dem Titel ‘{message_subject}’ gesendet .

Sie können die private Nachricht anzeigen, indem Sie sich anmelden und Ihren Posteingang anzeigen unter:
{site_url}
...
Deaktivieren Sie die Option in Ihren Email-Einstellungen, um keine Benachrichtigungen zu privaten Nachrichten mehr zu erhalten.
...
if ( ! function_exists('pm_inbox_full_title'))
{
	...
	{
return <<<EOF
Ihre private Nachrichtenmailbox ist voll
EOF;
	}
}
...
{sender_name} hat gerade versucht, Ihnen eine private Nachricht zu senden,
Ihr Posteingang ist jedoch voll und überschreitet das Maximum von {pm_storage_limit}.

Bitte melden Sie sich an und entfernen Sie unerwünschte Nachrichten aus Ihrem Posteingang unter:
{site_url}
EOF;
	}
}
...
if ( ! function_exists('forum_moderation_notification'))
{
	...
	{
return <<<EOF
{name_of_recipient}, ein Moderator hat {moderation_action} Ihren Thread.
...
if ( ! function_exists('forum_report_notification_title'))
{
	...
	{
return <<<EOF
Beitrag berichtet in {forum_name}
EOF;
	}
}
...
if ( ! function_exists('forum_report_notification'))
{
	...
	{
return <<<EOF
{reporter_name} hat gerade einen Beitrag von {author} in
{forum_name} geschrieben.

Der Grund für den Bericht:
{reasons}

Zusätzliche Hinweise von {reporter_name}:
{notes}

Der Beitrag ist zu finden unter:
{post_url}

Inhalt des gemeldeten Beitrags:
{body}
EOF;
	}
}
